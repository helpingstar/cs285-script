...
26. a pi st at is our advantage.
27. That's the difference between our q value and our value.
28. And intuitively the advantage says how much better is at than the average action according to the policy pi, where pi is the policy for which we calculated this advantage.
29. So then arg max with respect to at of a pi st at is the best action that we could take from st if we follow pi thereafter.
...
32. So if it's the best action from st if we then follow pi thereafter, then it's at least as good as whatever action pi would've chosen.
33. And the interesting thing is that this is true regardless of what pi actually is.
...
42. So each time we create this implicit pi-prime, we know that that is at least as good as pi and in most cases better.
43. So we still have an algorithm with the usual three boxes, where in the orange box are generated samples, in the green box we're going to fit some kind of value function, either q-pi or v-pi, which we will use to estimate the advantage, and in the green box we're going to fit some kind of value function, either q-pi or v-pi, and in the green box we're going to fit some kind of value function, either q-pi or v-pi, and in the blue box, instead of taking a gradient ascent step on an explicit policy, we will construct this implicit policy as the argmax.
...
46. So in policy iteration, step one is to evaluate the advantage of your current policy pi, and then step two is to construct a new policy that's going to be this pi prime, where pi prime takes an action with probability one if it is the argmax of the advantage.
...
51. So step one is to evaluate the advantage of pi, and step two is to check the advantage value of pi.
...
56. How do you evaluate the advantage a pi for a particular state action tuple for a given previous policy pi, which will also be an implicit policy, but we don't care so much about that right now.
57. So like before, we can express the advantage a pi sa as the reward sa plus gamma times the expected value of v pi at s prime minus v pi at s.
58. So let's try to evaluate v pi of s.
59. One way to evaluate v pi of s in order to then estimate these advantages for policy iteration is to use dynamic programming.
...
65. So when we do this, if we take the first step, we can now estimate the value of pi for the transition our entire state and action space.
...
70. So in this kind of small state space, you can actually store the full value function v pi in a table, right.
71. You can actually construct a table with 16 numbers and just write down the v pi for every one of those 16 numbers.
...
77. So if we want to update v pi of s, we can set it to be the expected value with respect to the actions a sampled from a policy pine of the reward s a plus gamma times the expected value over s prime sampled from p of s prime given s a of v pi s prime.
...
80. And then of course, we need to know v pi s prime.
...
83. And then once we calculate a value function v pi this way, then we can construct a better policy pi prime, as I mentioned before, by assigning probability 1 to the action that is the arc max of the advantage that we obtain from this value function.
84. Now this also means that our policy will be deterministic.
85. So expected values with respect to this pi will be pretty easy to calculate.
86. So we can also use this data to calculate the probability of the action.
87. So we can simplify our bootstrap update by removing the expectation with respect to pi and just directly plugging in the only action that has non-zero probability.
88. So then we get the simplified backup where v prime of s is set to r of s comma pi of s plus gamma times the expectation under p s prime given s pi of s of v pi s prime.
...
95. And then step two, set your new policy to be this pi prime policy obtained via the arc max.
96. And then repeat.
...
102. You can prove that repeating this recursion eventually converges to a fixed point, and this fixed point is the true value function v pi.
103. For those of you that are a bit more mathematically inclined, I will also point out that if you write v pi of s equals r of s pi s plus this expectation, that actually represents a system of linear equations that describe the value function v pi.
...
115. Now, if you remove the minus v pi s, you just get the q function.
